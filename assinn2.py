...
def function2():
    ...
    x = np.arange(10, 37, dtype=float).reshape(
...
def function9():
    ...
    a = np.arange(1, 101)[np.arange(1, 101) % 2 == 0]
...
def function12():
    ...
    arr = np.array([1, 2, 3])
    ans = np.hstack(
        (np.hstack((np.hstack(np.dstack((arr, arr, arr))))), arr, arr, arr))
...
def function15():
    ...
    ans = np.sort(arr, axis=0)  # write your code here
...
def function17():
    ...
    arr1 = np.full(100, "NO").reshape(10, 10).astype('U3')
    tru = (arr % 3 == 0) & (arr % 5 == 0)
    arr1[tru] = 'YES'
